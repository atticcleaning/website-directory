...
export default function ArticleCard({ title, excerpt, topicTag, slug }: ArticleCardProps) {
  return (
    <Link
      href={`/articles/${slug}`}
      className="block min-h-[44px] rounded-lg border border-border bg-card p-4 shadow-card transition-all duration-200 hover:shadow-card-hover motion-safe:hover:-translate-y-1 hover:border-primary/60"
    >
